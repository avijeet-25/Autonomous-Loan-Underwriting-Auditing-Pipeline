...
def check_internal_blacklist_registry(pan_card: str) -> str:
    ...
    if normalized_pan in flagged_registry:
        return json.dumps({
            "status": "FLAGGED",
            "reason": "Severe historical credit defaults, willful non-performing asset (NPA) match, or active fraud registry alert flag."
        })
    return json.dumps({
        "status": "CLEARED",
...
def calculate_financial_ratios(monthly_income: float, existing_debt: float, requested_amount: float, collateral_value: float) -> str:
    ...
    return json.dumps({
        ...
        "is_dti_critical": dti_percentage > 45.0,
        "is_ltv_safe": ltv_percentage <= 80.0
    })
